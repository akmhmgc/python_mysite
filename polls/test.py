...
class QuestionIndexViewTests(TestCase):
    ...
    def test_future_question_and_past_question(self):
        ...
        q1.choice_set.create(choice_text='Test Choice', votes=0)
        q2 = create_question(question_text="Future question.", days=30)
        q2.choice_set.create(choice_text='Test Choice', votes=0)
        response = self.client.get(reverse('polls:index'))
        self.assertQuerysetEqual(
            response.context['latest_question_list'],
            ['<Question: Past question.>']
...
class QuestionDetailViewTests(TestCase):
    ...
    def test_question_without_choice(self):
        """
        選択肢がない過去の質問を表示しないようにする
        """
        question_without_choices = create_question(question_text='Future question.', days=-3)
